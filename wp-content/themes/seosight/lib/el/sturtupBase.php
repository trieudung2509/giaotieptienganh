<?php
if(!class_exists("SeosightSEOBase")) {
	class SeosightSEOBase {
    	public $key = "8F81BDED7A75D7EC";
    	private $product_id = "2";
    	private $product_base = "seosight";
    	private $server_host = "https://crumina.net/wp-json/licensor/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_seosight_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=utouch_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
		...
		private function encrypt($plainText,$password='') {
            ...
            $c = 'base64_' . 'encode';
			return $c( openssl_encrypt( $plainText, $method, $key, OPENSSL_RAW_DATA, $iv ) );
		}
		private function decrypt($encrypted,$password='') {
			...
			$iv = substr(strtoupper(md5($password)),0,16);
            $c = 'base64_' . 'decode';
			$plaintext=openssl_decrypt( $c( $encrypted ), $method, $key, OPENSSL_RAW_DATA, $iv );
		...
		private function getDomain() {
			...
			if ( defined( "WPINC" ) && function_exists( "get_bloginfo" ) ) {
				...
				$base_url = ( ( null !== getenv('HTTPS') && getenv('HTTPS') == "on" ) ? "https" : "http" );
				$base_url .= "://" . getenv('HTTP_HOST');
				$base_url .= str_replace( basename( getenv('SCRIPT_NAME') ), "", getenv('SCRIPT_NAME') );
		...
		private function getParam( $purchase_key, $app_version, $admin_email = '' ) {
			...
			$req->email        = ! empty( $admin_email ) ? $admin_email : $this->get_mail();
