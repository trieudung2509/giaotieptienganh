...
class Seosight_Menu_Custom_Walker extends Walker_Nav_Menu {
	/**
...
# END - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	protected function sub_menu_contains_icons( $element, $children_elements ) {
		...
		$id       = $element->$id_field;
		foreach ( $children_elements[ $id ] as $child ) {
			$parsed_icn = get_post_meta($child, 'seosight_menu_icon', true);
	...
	protected function row_contains_icons( $row, $first_column, $children_elements ) {
		...
		while ( true ) {
			...
			$mega_fields_child = get_post_meta($child, 'seosight_menu_options', true);
